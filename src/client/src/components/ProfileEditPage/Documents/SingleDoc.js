    ...
    setUserEdit((prevState) => ({
      userEdit: {
        ...
        typeUser: {
          ...
          documents: [
            ...prevState.userEdit.typeUser.documents.filter(
              (documentName) => documentName !== document
            ),
          ],
          documentFiles: [
            ...prevState.userEdit.typeUser.documentFiles.filter(
  ...
  return (
    <div className="document_single">
      ...
      <a href={getDocumentURL(document)} target="_blank">
        {" "}
        {getDocumentURL(document).split("/").reverse()[0].length > 5
          ? getDocumentURL(document).split("/").reverse()[0].slice(0, 5) + "..."
          : getDocumentURL(document).split("/").reverse()[0]}
      </a>
